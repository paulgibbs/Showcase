 ...
 * vars 'dps_user_id' with the displayed user id and 'author_name' with the
 * displayed user's nicename are added.
 *
 * If it's a forum edit, WP_Query::dps_is_forum_edit is set to true
 * If it's a topic edit, WP_Query::dps_is_topic_edit is set to true
 * If it's a reply edit, WP_Query::dps_is_reply_edit is set to true.
 *
 * If it's a view page, WP_Query::dps_is_view is set to true
...
function dps_parse_query( $posts_query ) {
	...
	// Get query variables
	$dps_view = $posts_query->get( dps_get_view_rewrite_id() );
	$dps_user = $posts_query->get( dps_get_user_rewrite_id() );
	$is_edit  = $posts_query->get( dps_get_edit_rewrite_id() );

	// It is a user page - We'll also check if it is user edit
	if ( !empty( $dps_user ) ) {

		// Not a user_id so try email and slug
		if ( get_option( 'permalink_structure' ) || ! is_numeric( $dps_user ) ) {

			// Email was passed
			if ( is_email( $dps_user ) ) {
				$dps_user = get_user_by( 'email', $dps_user );

			// Try nicename
			} else {
				$dps_user = get_user_by( 'slug', $dps_user );
			}

			// If we were successful, set to ID
			if ( is_object( $dps_user ) ) {
				$dps_user = $dps_user->ID;
			}
		}

		// Cast as int, just in case
		$dps_user = (int) $dps_user;

		// 404 and bail if user does not have a profile
		if ( ! dps_user_has_profile( $dps_user ) ) {
			$posts_query->set_404();
			return;
		}
		...
		$is_favs    = $posts_query->get( dps_get_user_favorites_rewrite_id()     );
		$is_subs    = $posts_query->get( dps_get_user_subscriptions_rewrite_id() );
		$is_topics  = $posts_query->get( dps_get_user_topics_rewrite_id()        );
		$is_replies = $posts_query->get( dps_get_user_replies_rewrite_id()       );

		// View or edit?
		if ( !empty( $is_edit ) ) {

			// We are editing a profile
			$posts_query->dps_is_single_user_edit = true;
		...
		// User favorites
			...
			$posts_query->dps_is_single_user_favs = true;

		// User subscriptions
		} elseif ( ! empty( $is_subs ) ) {
			$posts_query->dps_is_single_user_subs = true;

		// User topics
		} elseif ( ! empty( $is_topics ) ) {
			$posts_query->dps_is_single_user_topics = true;

		// User topics
		} elseif ( ! empty( $is_replies ) ) {
			$posts_query->dps_is_single_user_replies = true;

		// User profile
		} else {
			$posts_query->dps_is_single_user_profile = true;
		}

		// Looking at a single user
		$posts_query->dps_is_single_user = true;
		...
		// Get the user data
		$user = get_userdata( $dps_user );

		// User is looking at their own profile
		if ( get_current_user_id() == $user->ID ) {
			$posts_query->dps_is_single_user_home = true;
		}

		// Set dps_user_id for future reference
		$posts_query->set( 'dps_user_id', $user->ID );
	...
	// View Page
		...
		$view_args = dps_get_view_query_args( $dps_view );
		...
		$posts_query->dps_is_view = true;

	// Search Page
	} elseif ( isset( $posts_query->query_vars[ dps_get_search_rewrite_id() ] ) ) {

		// Check if there are search query args set
		$search_terms = dps_get_search_terms();
		if ( !empty( $search_terms ) )
			$posts_query->dps_search_terms = $search_terms;

		// Correct is_home variable
		$posts_query->is_home = false;

		// We are in a search query
		$posts_query->dps_is_search = true;
	...
	// Forum/Topic/Reply Edit Page
		...
		if ( !empty( $post_type ) ) {
			switch( $post_type ) {
				...
				case dps_get_topic_post_type() :
					$posts_query->dps_is_topic_edit = true;
					$posts_query->dps_is_edit       = true;
					break;

				// We are editing a reply
				case dps_get_reply_post_type() :
					$posts_query->dps_is_reply_edit = true;
					$posts_query->dps_is_edit       = true;
					break;
			}
		...
		// We are editing a topic tag
			...
			$posts_query->dps_is_topic_tag_edit = true;
			$posts_query->dps_is_edit           = true;
		}
	...
	// Topic tag page
		...
		$posts_query->set( 'dps_topic_tag',  get_query_var( 'term' )   );
		$posts_query->set( 'post_type',      dps_get_topic_post_type() );
		$posts_query->set( 'posts_per_page', dps_get_topics_per_page() );
	}
}
