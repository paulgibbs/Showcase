 ...
 * @since Showcase (1.0)
 *
 * @uses do_action() Calls 'dps_clean_post_cache' on $id
 * @param object|int $_post The post object or ID to remove from the cache
 */
function dps_clean_post_cache( $_post = '' ) {
	...
	do_action( 'dps_clean_post_cache', $_post->ID, $_post );
	...
	if ( ! empty( $_post->post_parent ) ) {
		dps_clean_post_cache( $_post->post_parent );
	}
}
