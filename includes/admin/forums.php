...
class BB_Forums_Admin {
	...
	private function setup_actions() {
		...
		add_action( 'dps_admin_head',        array( $this, 'admin_head'       ) );
	...
	public function attributes_metabox() {
		...
		do_action( 'dps_forum_attributes_metabox' );
	}
	 ...
	 * @uses dps_close_forum() To close the forum
	 * @uses dps_open_forum() To open the forum
	 * @uses dps_categorize_forum() To make the forum a category
	 * @uses dps_normalize_forum() To make the forum normal (not category)
	 * @uses dps_privatize_forum() To mark the forum as private
	 * @uses dps_publicize_forum() To mark the forum as public
	 * @uses do_action() Calls 'dps_forum_attributes_metabox_save' with the
	 *                    forum id
	 * @return int Forum id
	 */
	public function attributes_metabox_save( $forum_id ) {
		...
		// Nonce check
		if ( empty( $_POST['dps_forum_metabox'] ) || !wp_verify_nonce( $_POST['dps_forum_metabox'], 'dps_forum_metabox_save' ) )
			return $forum_id;
		...
		do_action( 'dps_forum_attributes_metabox_save', $forum_id );
	...
	public function admin_head() {
		...
		/*<![CDATA[*/
			...
			#dps_forum_attributes hr {
				border-style: solid;
				border-width: 1px;
				border-color: #ccc #fff #fff #ccc;
			}

			.column-dps_forum_topic_count,
			.column-dps_forum_reply_count,
			.column-dps_topic_reply_count,
			.column-dps_topic_voice_count {
				width: 8% !important;
			}

			.column-author,
			.column-dps_reply_author,
			.column-dps_topic_author {
				width: 10% !important;
			}

			.column-dps_topic_forum,
			.column-dps_reply_forum,
			.column-dps_reply_topic {
				width: 10% !important;
			}

			.column-dps_forum_freshness,
			.column-dps_topic_freshness {
				width: 10% !important;
			}

			.column-dps_forum_created,
			.column-dps_topic_created,
			.column-dps_reply_created {
				width: 15% !important;
			}
	 ...
	 * @param int $forum_id Forum id
	 * @uses dps_forum_topic_count() To output the forum topic count
	 * @uses dps_forum_reply_count() To output the forum reply count
	 ...
	 * @uses dps_get_forum_last_active_time() To get the time when the forum was
	 *                                    last active
	 * @uses do_action() Calls 'dps_admin_forums_column_data' with the
	 *                    column and forum id
	 */
	public function column_data( $column, $forum_id ) {
		...
		switch ( $column ) {
			case 'dps_forum_topic_count' :
				dps_forum_topic_count( $forum_id );
				break;

			case 'dps_forum_reply_count' :
				dps_forum_reply_count( $forum_id );
				break;
			...
			case 'dps_forum_freshness' :
				$last_active = dps_get_forum_last_active_time( $forum_id, false );
			...
			default:
				do_action( 'dps_admin_forums_column_data', $column, $forum_id );
				break;
		}
	}
	...
	public function updated_messages( $messages ) {
		...
		$forum_url = dps_get_forum_permalink( $post_ID );

		// Current forum's post_date
		$post_date = dps_get_global_post_field( 'post_date', 'raw' );
