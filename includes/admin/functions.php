		...
		if ( $second_sep == $item ) {
			...
			foreach( $custom_menus as $custom_menu ) {
				if ( array_search( $custom_menu, $menu_order ) ) {
					$dps_menu_order[] = $custom_menu;
				}
			}

			// Add the appearance separator
			$dps_menu_order[] = $second_sep;
 ...
 * @uses dps_is_custom_post_type() To get the forum post type
 *
 * @return string The custom post type permalink
 */
function dps_filter_sample_permalink( $post_link, $_post, $leavename = false, $sample = false ) {

	// Bail if not on an admin page and not getting a sample permalink
	if ( !empty( $sample ) && is_admin() && dps_is_custom_post_type() )
