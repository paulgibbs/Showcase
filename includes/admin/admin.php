...
class BB_Admin {
	...
	private function setup_actions() {
		...
		add_action( 'dps_admin_head',              array( $this, 'admin_head'                 ) ); // Add some general styling to the admin area
		add_action( 'dps_admin_notices',           array( $this, 'activation_notice'          ) ); // Add notice if not using a barebones theme
		...
		add_filter( 'dps_map_meta_caps',   array( $this, 'map_settings_meta_caps' ), 10, 4 );
	...
	public function admin_menus() {
		...
		if ( current_user_can( 'dps_tools_page' ) ) {
			if ( current_user_can( 'dps_tools_repair_page' ) ) {
			...
			if ( current_user_can( 'dps_tools_import_page' ) ) {
				$hooks[] = add_management_page(
					__( 'Import Forums', 'barebones' ),
					__( 'Forum Import',  'barebones' ),
					$this->minimum_capability,
					'bbp-converter',
					'dps_converter_settings'
				);
			}

			if ( current_user_can( 'dps_tools_reset_page' ) ) {
				$hooks[] = add_management_page(
			...
			foreach( $hooks as $hook ) {
				add_action( "admin_head-$hook", 'dps_tools_modify_menu_highlight' );
			}
		...
		// These are later removed in admin_head
		if ( current_user_can( 'dps_about_page' ) ) {
	...
	public function register_importers() {
		...
		// Load our importers
		$importers = apply_filters( 'dps_importers', array( 'barebones' ) );
	...
	public static function dashboard_widget_right_now() {
		wp_add_dashboard_widget( 'bbp-dashboard-right-now', __( 'Right Now in Forums', 'barebones' ), 'dps_dashboard_widget_right_now' );
	}
	...
	public function admin_head() {
		...
		$forum_class = sanitize_html_class( dps_get_forum_post_type() );
		$topic_class = sanitize_html_class( dps_get_topic_post_type() );
		$reply_class = sanitize_html_class( dps_get_reply_post_type() );

		if ( ( 'post' == get_current_screen()->base ) && ( dps_get_reply_post_type() == get_current_screen()->post_type ) ) : ?>

		<script type="text/javascript">
			jQuery(document).ready(function() {

				var dps_topic_id = jQuery( '#dps_topic_id' );

				dps_topic_id.suggest( ajaxurl + '?action=dps_suggest_topic', {
					onSelect: function() {
						var value = this.value;
						dps_topic_id.val( value.substr( 0, value.indexOf( ' ' ) ) );
					}
				} );
			});
		</script>
		...
		/*<![CDATA[*/
			...
			#poststuff #dps_forum_attributes select#parent_id,
			#poststuff #dps_topic_attributes select#parent_id,
			#poststuff #dps_reply_attributes select#dps_forum_id {
				max-width: 170px;
			}
	...
	public function suggest_topic() {
		...
		if ( ! empty( $topics ) ) {
			foreach ( (array) $topics as $post ) {
				echo sprintf( __( '%s - %s', 'barebones' ), dps_get_topic_id( $post->ID ), dps_get_topic_title( $post->ID ) ) . "\n";
			}
		}
		die();
	}
