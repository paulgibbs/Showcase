 ...
 * @since Showcase (1.0)
 *
 * @uses apply_filters() Calls 'dps_get_forum_caps' with the capabilities
 * @return array Forum capabilities
 */
function dps_get_forum_caps() {
	return apply_filters( 'dps_get_forum_caps', array (
 ...
 */
function dps_map_forum_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'read_forum' :
			...
			// Do some post ID based logic
				...
				if ( !empty( $_post ) ) {
					...
					// Post is public
					if ( dps_get_public_status_id() == $_post->post_status ) {
						$caps = array( 'spectate' );
		...
		case 'edit_forum' :
			...
			if ( !empty( $_post ) ) {
				...
				if ( dps_is_user_inactive( $user_id ) ) {
					$caps[] = 'do_not_allow';
		...
		case 'dps_forums_admin' :
			$caps = array( 'keep_gate' );
			break;
	}

	return apply_filters( 'dps_map_forum_meta_caps', $caps, $cap, $user_id, $args );
}
