 ...
 */
function dps_insert_forum( $forum_data = array(), $forum_meta = array() ) {
	...
	$forum_data = dps_parse_args( $forum_data, $default_forum, 'insert_forum' );
	...
	$forum_meta = dps_parse_args( $forum_meta, $default_meta, 'insert_forum_meta' );

	// Insert forum meta
	foreach ( $forum_meta as $meta_key => $meta_value )
		update_post_meta( $forum_id, '_dps_' . $meta_key, $meta_value );
...
function dps_new_forum_handler( $action = '' ) {
	...
	$forum_author = dps_get_current_user_id();

	// Remove wp_filter_kses filters from title and content for capable users and if the nonce is verified
	if ( current_user_can( 'unfiltered_html' ) && !empty( $_POST['_dps_unfiltered_html_forum'] ) && wp_create_nonce( 'bbp-unfiltered-html-forum_new' ) == $_POST['_dps_unfiltered_html_forum'] ) {
		remove_filter( 'dps_new_forum_pre_title',   'wp_filter_kses' );
		remove_filter( 'dps_new_forum_pre_content', 'wp_filter_kses' );
	}

	/** Forum Title ***********************************************************/

	if ( !empty( $_POST['dps_forum_title'] ) )
		$forum_title = esc_attr( strip_tags( $_POST['dps_forum_title'] ) );

	// Filter and sanitize
	$forum_title = apply_filters( 'dps_new_forum_pre_title', $forum_title );
	...
	if ( !empty( $_POST['dps_forum_content'] ) )
		$forum_content = $_POST['dps_forum_content'];

	// Filter and sanitize
	$forum_content = apply_filters( 'dps_new_forum_pre_content', $forum_content );
	...
	if ( !empty( $_POST['dps_forum_parent_id'] ) )
		$forum_parent_id = (int) $_POST['dps_forum_parent_id'];

	// Filter and sanitize
	$forum_parent_id = apply_filters( 'dps_new_forum_pre_parent_id', $forum_parent_id );
	...
	$post_status = dps_get_public_status_id();
	if ( !dps_check_for_moderation( $anonymous_data, $forum_author, $forum_title, $forum_content ) )
		$post_status = dps_get_pending_status_id();

	/** Additional Actions (Before Save) **************************************/

	do_action( 'dps_new_forum_pre_extras', $forum_parent_id );
	...
	$forum_data = apply_filters( 'dps_new_forum_pre_insert', array(
	...
	if ( !empty( $forum_id ) && !is_wp_error( $forum_id ) ) {
		...
		if ( ( get_post_field( 'post_status', $forum_id ) == dps_get_trash_status_id() ) || ( $forum_data['post_status'] == dps_get_trash_status_id() ) ) {
		...
		if ( $forum_data['post_status'] == dps_get_spam_status_id() ) {
			add_post_meta( $forum_id, '_dps_spam_meta_status', dps_get_public_status_id() );
		...
		$forum_args = array(
			...
			'last_active_id'     => 0,
			'last_active_time'   => 0,
			'last_active_status' => dps_get_public_status_id()
		);
		do_action( 'dps_new_forum', $forum_args );

		/** Additional Actions (After Save) ***********************************/

		do_action( 'dps_new_forum_post_extras', $forum_id );

		/** Redirect **********************************************************/

		// Redirect to
		$redirect_to  = dps_get_redirect_to();

		// Get the forum URL
		$redirect_url = dps_get_forum_permalink( $forum_id, $redirect_to );

		// Add view all?
		if ( dps_get_view_all() || !empty( $view_all ) ) {

			// User can moderate, so redirect to forum with view all set
			if ( current_user_can( 'moderate' ) ) {
				$redirect_url = dps_add_view_all( $redirect_url );

			// User cannot moderate, so redirect to forum
			} else {
				$redirect_url = dps_get_forum_permalink( $forum_id );
			}
		}

		// Allow to be filtered
		$redirect_url = apply_filters( 'dps_new_forum_redirect_to', $redirect_url, $redirect_to );
 ...
 * @uses dps_is_forum_private() To check if the forum is private
 * @uses remove_filter() To remove 'wp_filter_kses' filters if needed
 * @uses apply_filters() Calls 'dps_edit_forum_pre_title' with the title and
 *                        forum id
 * @uses apply_filters() Calls 'dps_edit_forum_pre_content' with the content
 *                        and forum id
 * @uses Showcase::errors::get_error_codes() To get the {@link WP_Error} errors
 * @uses wp_save_post_revision() To save a forum revision
 * @uses dps_update_forum_revision_log() To update the forum revision log
 * @uses wp_update_post() To update the forum
 * @uses do_action() Calls 'dps_edit_forum' with the forum id, forum id,
 *                    anonymous data and reply author
 * @uses dps_move_forum_handler() To handle movement of a forum from one forum
 *                                 to another
 * @uses dps_get_forum_permalink() To get the forum permalink
 * @uses wp_safe_redirect() To redirect to the forum link
 * @uses Showcase::errors::get_error_messages() To get the {@link WP_Error} error
 *                                              messages
 */
function dps_edit_forum_handler( $action = '' ) {
	...
	// Forum id was passed
	} elseif ( is_numeric( $_POST['dps_forum_id'] ) ) {
		$forum_id = (int) $_POST['dps_forum_id'];
	...
	// Remove wp_filter_kses filters from title and content for capable users and if the nonce is verified
	if ( current_user_can( 'unfiltered_html' ) && !empty( $_POST['_dps_unfiltered_html_forum'] ) && ( wp_create_nonce( 'bbp-unfiltered-html-forum_' . $forum_id ) == $_POST['_dps_unfiltered_html_forum'] ) ) {
		remove_filter( 'dps_edit_forum_pre_title',   'wp_filter_kses' );
		remove_filter( 'dps_edit_forum_pre_content', 'wp_filter_kses' );
	}

	/** Forum Parent ***********************************************************/

	// Forum parent id was passed
	if ( is_numeric( $_POST['dps_forum_parent_id'] ) ) {
		$forum_parent_id = (int) $_POST['dps_forum_parent_id'];
	}

	// Current forum this forum is in
	$current_parent_forum_id = dps_get_forum_parent_id( $forum_id );
	...
	// Filter and sanitize
	$forum_title = apply_filters( 'dps_edit_forum_pre_title', $forum_title, $forum_id );
	...
	// Filter and sanitize
	$forum_content = apply_filters( 'dps_edit_forum_pre_content', $forum_content, $forum_id );
	...
	$post_status = dps_get_public_status_id();
	if ( !dps_check_for_moderation( $anonymous_data, dps_get_forum_author_id( $forum_id ), $forum_title, $forum_content ) )
		$post_status = dps_get_pending_status_id();

	/** Additional Actions (Before Save) **************************************/

	do_action( 'dps_edit_forum_pre_extras', $forum_id );
	...
	$forum_data = apply_filters( 'dps_edit_forum_pre_insert', array(
		'ID'           => $forum_id,
		'post_title'   => $forum_title,
		'post_content' => $forum_content,
		'post_status'  => $post_status,
	...
	// Revision Reason
	if ( !empty( $_POST['dps_forum_edit_reason'] ) )
		$forum_edit_reason = esc_attr( strip_tags( $_POST['dps_forum_edit_reason'] ) );

	// Update revision log
	if ( !empty( $_POST['dps_log_forum_edit'] ) && ( 1 == $_POST['dps_log_forum_edit'] ) && ( $revision_id = wp_save_post_revision( $forum_id ) ) ) {
		dps_update_forum_revision_log( array(
			'forum_id'    => $forum_id,
			'revision_id' => $revision_id,
			'author_id'   => dps_get_current_user_id(),
	...
	if ( !empty( $forum_id ) && !is_wp_error( $forum_id ) ) {
		...
		do_action( 'dps_edit_forum', $forum_args );

		// If the new forum parent id is not equal to the old forum parent
		// id, run the dps_move_forum action and pass the forum's parent id
		// as the first arg and new forum parent id as the second.
		// @todo implement
		//if ( $forum_id != $forum->post_parent )
		//	dps_move_forum_handler( $forum_parent_id, $forum->post_parent, $forum_id );

		/** Additional Actions (After Save) ***********************************/

		do_action( 'dps_edit_forum_post_extras', $forum_id );
		...
		// View all?
		$view_all = dps_get_view_all();

		// Get the forum URL
		$forum_url = dps_get_forum_permalink( $forum_id, $redirect_to );

		// Add view all?
		if ( !empty( $view_all ) )
			$forum_url = dps_add_view_all( $forum_url );

		// Allow to be filtered
		$forum_url = apply_filters( 'dps_edit_forum_redirect_to', $forum_url, $view_all, $redirect_to );
 ...
 * @since Showcase (1.0)
 * @param int $forum_id
 * @uses dps_is_forum_closed() To check if forum is closed
 * @uses dps_close_forum() To close forum
 * @uses dps_open_forum() To open forum
 * @uses dps_is_forum_category() To check if forum is a category
 * @uses dps_categorize_forum() To turn forum into a category
 * @uses dps_normalize_forum() To turn category into forum
 * @uses dps_get_public_status_id() To get the public status ID
 * @uses dps_get_private_status_id() To get the private status ID
 * @uses dps_get_hidden_status_id() To get the hidden status ID
 * @uses dps_get_forum_visibility() To get the forums visibility
 * @uses dps_hide_forum() To hide a forum
 * @uses dps_privatize_forum() To make a forum private
 * @uses dps_publicize_forum() To make a forum public
 * @return If forum ID is empty
 */
function dps_save_forum_extras( $forum_id = 0 ) {

	// Validate the forum ID
	$forum_id = dps_get_forum_id( $forum_id );

	// Bail if forum ID is empty
	if ( empty( $forum_id ) || ! dps_is_forum( $forum_id ) )
		return;

	/** Forum Status ******************************************************/

	if ( !empty( $_POST['dps_forum_status'] ) && in_array( $_POST['dps_forum_status'], array( 'open', 'closed' ) ) ) {
		if ( 'closed' == $_POST['dps_forum_status'] && !dps_is_forum_closed( $forum_id, false ) ) {
			dps_close_forum( $forum_id );
		} elseif ( 'open' == $_POST['dps_forum_status'] && dps_is_forum_closed( $forum_id, false ) ) {
			dps_open_forum( $forum_id );
		}
	}

	/** Forum Type ********************************************************/

	if ( !empty( $_POST['dps_forum_type'] ) && in_array( $_POST['dps_forum_type'], array( 'forum', 'category' ) ) ) {
		if ( 'category' == $_POST['dps_forum_type'] && !dps_is_forum_category( $forum_id ) ) {
			dps_categorize_forum( $forum_id );
		} elseif ( 'forum' == $_POST['dps_forum_type'] && dps_is_forum_category( $forum_id ) ) {
			dps_normalize_forum( $forum_id );
		}
	}

	/** Forum Visibility **************************************************/

	if ( !empty( $_POST['dps_forum_visibility'] ) && in_array( $_POST['dps_forum_visibility'], array( dps_get_public_status_id(), dps_get_private_status_id(), dps_get_hidden_status_id() ) ) ) {

		// Get forums current visibility
		$visibility = dps_get_forum_visibility( $forum_id );

		// What is the new forum visibility setting?
		switch ( $_POST['dps_forum_visibility'] ) {

			// Hidden
			case dps_get_hidden_status_id()  :
				dps_hide_forum( $forum_id, $visibility );
				break;

			// Private
			case dps_get_private_status_id() :
				dps_privatize_forum( $forum_id, $visibility );
				break;

			// Publish (default)
			case dps_get_public_status_id()  :
			default        :
				dps_publicize_forum( $forum_id, $visibility );
				break;
		}
	}
}
 ...
 * @uses do_action() Calls 'dps_close_forum' with the forum id
 * @uses update_post_meta() To add the previous status to a meta
 * @uses do_action() Calls 'dps_opened_forum' with the forum id
 * @return mixed False or {@link WP_Error} on failure, forum id on success
 */
function dps_close_forum( $forum_id = 0 ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_close_forum',  $forum_id );

	update_post_meta( $forum_id, '_dps_status', 'closed' );

	do_action( 'dps_closed_forum', $forum_id );
 ...
 *
 * @param int $forum_id forum id
 * @uses do_action() Calls 'dps_open_forum' with the forum id
 * @uses get_post_meta() To get the previous status
 * @uses update_post_meta() To delete the previous status meta
 * @uses do_action() Calls 'dps_opened_forum' with the forum id
 * @return mixed False or {@link WP_Error} on failure, forum id on success
 */
function dps_open_forum( $forum_id = 0 ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_open_forum',   $forum_id );

	update_post_meta( $forum_id, '_dps_status', 'open' );

	do_action( 'dps_opened_forum', $forum_id );
 ...
 */
function dps_categorize_forum( $forum_id = 0 ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_categorize_forum',  $forum_id );

	update_post_meta( $forum_id, '_dps_forum_type', 'category' );

	do_action( 'dps_categorized_forum', $forum_id );
 ...
 * @return bool False on failure, true on success
 */
function dps_normalize_forum( $forum_id = 0 ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_normalize_forum',  $forum_id );

	update_post_meta( $forum_id, '_dps_forum_type', 'forum' );

	do_action( 'dps_normalized_forum', $forum_id );
 ...
 * @return bool False on failure, true on success
 */
function dps_publicize_forum( $forum_id = 0, $current_visibility = '' ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_publicize_forum',  $forum_id );

	// Get private forums
	$private = dps_get_private_forum_ids();
	...
	if ( in_array( $forum_id, $private ) ) {
		...
		update_option( '_dps_private_forums', array_unique( array_filter( array_values( $private ) ) ) );
	}

	// Get hidden forums
	$hidden = dps_get_hidden_forum_ids();
	...
	if ( in_array( $forum_id, $hidden ) ) {
		...
		update_option( '_dps_hidden_forums', array_unique( array_filter( array_values( $hidden ) ) ) );
	}

	// Only run queries if visibility is changing
	if ( dps_get_public_status_id() != $current_visibility ) {

		// Update forum post_status
		global $wpdb;
		$wpdb->update( $wpdb->posts, array( 'post_status' => dps_get_public_status_id() ), array( 'ID' => $forum_id ) );
		wp_transition_post_status( dps_get_public_status_id(), $current_visibility, get_post( $forum_id ) );
	}

	do_action( 'dps_publicized_forum', $forum_id );
 ...
 * @return bool False on failure, true on success
 */
function dps_privatize_forum( $forum_id = 0, $current_visibility = '' ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_privatize_forum',  $forum_id );

	// Only run queries if visibility is changing
	if ( dps_get_private_status_id() != $current_visibility ) {
		...
		// Add to '_dps_private_forums' site option
		$private   = dps_get_private_forum_ids();
		$private[] = $forum_id;
		update_option( '_dps_private_forums', array_unique( array_filter( array_values( $private ) ) ) );
		...
		global $wpdb;
		$wpdb->update( $wpdb->posts, array( 'post_status' => dps_get_private_status_id() ), array( 'ID' => $forum_id ) );
		wp_transition_post_status( dps_get_private_status_id(), $current_visibility, get_post( $forum_id ) );
	}

	do_action( 'dps_privatized_forum', $forum_id );
 ...
 * @return bool False on failure, true on success
 */
function dps_hide_forum( $forum_id = 0, $current_visibility = '' ) {

	$forum_id = dps_get_forum_id( $forum_id );

	do_action( 'dps_hide_forum', $forum_id );

	// Only run queries if visibility is changing
	if ( dps_get_hidden_status_id() != $current_visibility ) {
		...
		// Add to '_dps_hidden_forums' site option
		$hidden   = dps_get_hidden_forum_ids();
		$hidden[] = $forum_id;
		update_option( '_dps_hidden_forums', array_unique( array_filter( array_values( $hidden ) ) ) );
		...
		// Update forums visibility setting
		global $wpdb;
		$wpdb->update( $wpdb->posts, array( 'post_status' => dps_get_hidden_status_id() ), array( 'ID' => $forum_id ) );
		wp_transition_post_status( dps_get_hidden_status_id(), $current_visibility, get_post( $forum_id ) );
	}

	do_action( 'dps_hid_forum',  $forum_id );
 ...
 * @uses apply_filters() Calls 'dps_bump_forum_topic_count' with the topic
 *                        count, forum id, and difference
 * @return int Forum topic count
 */
function dps_bump_forum_topic_count( $forum_id = 0, $difference = 1, $update_ancestors = true ) {

	// Get some counts
	$forum_id          = dps_get_forum_id( $forum_id );
	$topic_count       = dps_get_forum_topic_count( $forum_id, false, true );
	$total_topic_count = dps_get_forum_topic_count( $forum_id, true,  true );

	// Update this forum id
	update_post_meta( $forum_id, '_dps_topic_count',       (int) $topic_count       + (int) $difference );
	update_post_meta( $forum_id, '_dps_total_topic_count', (int) $total_topic_count + (int) $difference );
	...
	if ( true === $update_ancestors ) {
		...
		if ( !empty( $ancestors ) ) {
			foreach ( (array) $ancestors as $parent_forum_id ) {
				...
				// Get forum counts
				$parent_topic_count       = dps_get_forum_topic_count( $parent_forum_id, false, true );
				$parent_total_topic_count = dps_get_forum_topic_count( $parent_forum_id, true,  true );

				// Update counts
				update_post_meta( $parent_forum_id, '_dps_topic_count',       (int) $parent_topic_count       + (int) $difference );
				update_post_meta( $parent_forum_id, '_dps_total_topic_count', (int) $parent_total_topic_count + (int) $difference );
			}
		}
	}

	return (int) apply_filters( 'dps_bump_forum_topic_count', (int) $total_topic_count + (int) $difference, $forum_id, (int) $difference, (bool) $update_ancestors );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses update_post_meta() To update the forum's topic count meta
 * @uses apply_filters() Calls 'dps_bump_forum_topic_count_hidden' with the
 *                        topic count, forum id, and difference
 * @return int Forum hidden topic count
 */
function dps_bump_forum_topic_count_hidden( $forum_id = 0, $difference = 1 ) {

	// Get some counts
	$forum_id    = dps_get_forum_id( $forum_id );
	$topic_count = dps_get_forum_topic_count_hidden( $forum_id, true );
	$new_count   = (int) $topic_count + (int) $difference;

	// Update this forum id
	update_post_meta( $forum_id, '_dps_topic_count_hidden', (int) $new_count );

	return (int) apply_filters( 'dps_bump_forum_topic_count_hidden', (int) $new_count, $forum_id, (int) $difference );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses update_post_meta() To update the forum's topic count meta
 * @uses apply_filters() Calls 'dps_bump_forum_reply_count' with the topic
 *                        count, forum id, and difference
 * @return int Forum topic count
 */
function dps_bump_forum_reply_count( $forum_id = 0, $difference = 1, $update_ancestors = true ) {

	// Get some counts
	$forum_id          = dps_get_forum_id( $forum_id );
	$topic_count       = dps_get_forum_reply_count( $forum_id, false, true );
	$total_reply_count = dps_get_forum_reply_count( $forum_id, true,  true );

	// Update this forum id
	update_post_meta( $forum_id, '_dps_reply_count',       (int) $topic_count       + (int) $difference );
	update_post_meta( $forum_id, '_dps_total_reply_count', (int) $total_reply_count + (int) $difference );
	...
	if ( true === $update_ancestors ) {
		...
		if ( !empty( $ancestors ) ) {
			foreach ( (array) $ancestors as $parent_forum_id ) {
				...
				// Get forum counts
				$parent_topic_count       = dps_get_forum_reply_count( $parent_forum_id, false, true );
				$parent_total_reply_count = dps_get_forum_reply_count( $parent_forum_id, true,  true );

				// Update counts
				update_post_meta( $parent_forum_id, '_dps_reply_count',       (int) $parent_topic_count       + (int) $difference );
				update_post_meta( $parent_forum_id, '_dps_total_reply_count', (int) $parent_total_reply_count + (int) $difference );
			}
		}
	}

	return (int) apply_filters( 'dps_bump_forum_reply_count', (int) $total_reply_count + (int) $difference, $forum_id, (int) $difference, (bool) $update_ancestors );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_forum_query_subforum_ids() To get the subforum ids
 * @uses dps_update_forum_last_topic_id() To update the last topic id of child
 *                                         forums
 * @uses get_posts() To get the most recent topic in the forum
 * @uses update_post_meta() To update the forum's last active id meta
 * @uses apply_filters() Calls 'dps_update_forum_last_topic_id' with the last
 *                        reply id and forum id
 * @return bool True on success, false on failure
 */
function dps_update_forum_last_topic_id( $forum_id = 0, $topic_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );
	...
	if ( empty( $topic_id ) ) {
		...
		$children = dps_forum_query_subforum_ids( $forum_id );
		if ( !empty( $children ) ) {
			foreach ( (array) $children as $child ) {
				$children_last_topic = dps_update_forum_last_topic_id( $child ); // Recursive
			}
		}
		...
		$post_vars = array(
			...
			'meta_key'    => '_dps_last_active_time',
			'orderby'     => 'meta_value',
			'numberposts' => 1
		);
	...
	if ( dps_is_topic_published( $topic_id ) )
		update_post_meta( $forum_id, '_dps_last_topic_id', $topic_id );

	return (int) apply_filters( 'dps_update_forum_last_topic_id', $topic_id, $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_forum_query_subforum_ids() To get the subforum ids
 * @uses dps_update_forum_last_reply_id() To update the last reply id of child
 *                                         forums
 * @uses dps_forum_query_topic_ids() To get the topic ids in the forum
 * @uses dps_forum_query_last_reply_id() To get the forum's last reply id
 * @uses dps_is_reply_published() To make sure the reply is published
 * @uses update_post_meta() To update the forum's last active id meta
 * @uses apply_filters() Calls 'dps_update_forum_last_reply_id' with the last
 *                        reply id and forum id
 * @return bool True on success, false on failure
 */
function dps_update_forum_last_reply_id( $forum_id = 0, $reply_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );
	...
	if ( empty( $reply_id ) ) {
		...
		if ( !empty( $children ) ) {
			foreach ( (array) $children as $child ) {
				$children_last_reply = dps_update_forum_last_reply_id( $child ); // Recursive
			}
		}

		// If this forum has topics...
		$topic_ids = dps_forum_query_topic_ids( $forum_id );
		if ( !empty( $topic_ids ) ) {

			// ...get the most recent reply from those topics...
			$reply_id = dps_forum_query_last_reply_id( $forum_id, $topic_ids );
	...
	if ( dps_is_reply_published( $reply_id ) )
		update_post_meta( $forum_id, '_dps_last_reply_id', $reply_id );

	return (int) apply_filters( 'dps_update_forum_last_reply_id', $reply_id, $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_forum_query_subforum_ids() To get the subforum ids
 * @uses dps_update_forum_last_active_id() To update the last active id of
 *                                          child forums
 * @uses dps_forum_query_topic_ids() To get the topic ids in the forum
 * @uses dps_forum_query_last_reply_id() To get the forum's last reply id
 * @uses get_post_status() To make sure the reply is published
 * @uses update_post_meta() To update the forum's last active id meta
 * @uses apply_filters() Calls 'dps_update_forum_last_active_id' with the last
 *                        active post id and forum id
 * @return bool True on success, false on failure
 */
function dps_update_forum_last_active_id( $forum_id = 0, $active_id = 0 ) {
	...
	if ( empty( $active_id ) ) {
		...
		if ( !empty( $children ) ) {
			foreach ( (array) $children as $child ) {
				$children_last_active = dps_update_forum_last_active_id( $child, $active_id );
			}
		}
		...
		$topic_ids = dps_forum_query_topic_ids( $forum_id );
		if ( !empty( $topic_ids ) ) {
			$active_id = dps_forum_query_last_reply_id( $forum_id, $topic_ids );
	...
	if ( dps_get_public_status_id() == get_post_status( $active_id ) )
		update_post_meta( $forum_id, '_dps_last_active_id', (int) $active_id );

	return (int) apply_filters( 'dps_update_forum_last_active_id', (int) $active_id, $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_get_forum_last_active_id() To get the forum's last post id
 ...
 * @uses apply_filters() Calls 'dps_update_forum_last_active' with the new time
 *                        and forum id
 * @return bool True on success, false on failure
 */
function dps_update_forum_last_active_time( $forum_id = 0, $new_time = '' ) {
	$forum_id = dps_get_forum_id( $forum_id );

	// Check time and use current if empty
	if ( empty( $new_time ) )
		$new_time = get_post_field( 'post_date', dps_get_forum_last_active_id( $forum_id ) );

	// Update only if there is a time
	if ( !empty( $new_time ) )
		update_post_meta( $forum_id, '_dps_last_active_time', $new_time );

	return (int) apply_filters( 'dps_update_forum_last_active', $new_time, $forum_id );
}
 ...
 * @return bool True on success, false on failure
 */
function dps_update_forum_subforum_count( $forum_id = 0, $subforums = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $subforums ) )
		$subforums = count( dps_forum_query_subforum_ids( $forum_id ) );

	update_post_meta( $forum_id, '_dps_forum_subforum_count', (int) $subforums );

	return (int) apply_filters( 'dps_update_forum_subforum_count', (int) $subforums, $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_forum_query_subforum_ids() To get the subforum ids
 * @uses dps_update_forum_topic_count() To update the forum topic count
 * @uses dps_forum_query_topic_ids() To get the forum topic ids
 * @uses update_post_meta() To update the forum's topic count meta
 * @uses apply_filters() Calls 'dps_update_forum_topic_count' with the topic
 *                        count and forum id
 * @return int Forum topic count
 */
function dps_update_forum_topic_count( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );
	...
	if ( !empty( $children ) ) {
		foreach ( (array) $children as $child ) {
			$children_topic_count += dps_update_forum_topic_count( $child ); // Recursive
		}
	}

	// Get total topics for this forum
	$topics = (int) count( dps_forum_query_topic_ids( $forum_id ) );

	// Calculate total topics in this forum
	$total_topics = $topics + $children_topic_count;

	// Update the count
	update_post_meta( $forum_id, '_dps_topic_count',       (int) $topics       );
	update_post_meta( $forum_id, '_dps_total_topic_count', (int) $total_topics );

	return (int) apply_filters( 'dps_update_forum_topic_count', (int) $total_topics, $forum_id );
}
 ...
 * @uses dps_is_topic() To check if the supplied id is a topic
 * @uses dps_get_topic_id() To get the topic id
 ...
 * @uses apply_filters() Calls 'dps_update_forum_topic_count_hidden' with the
 *                        hidden topic count and forum id
 * @return int Topic hidden topic count
 */
function dps_update_forum_topic_count_hidden( $forum_id = 0, $topic_count = 0 ) {
	global $wpdb;

	// If topic_id was passed as $forum_id, then get its forum
	if ( dps_is_topic( $forum_id ) ) {
		$topic_id = dps_get_topic_id( $forum_id );
	...
	if ( !empty( $forum_id ) ) {
		...
		if ( empty( $topic_count ) )
			$topic_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_parent = %d AND post_status IN ( '" . join( '\',\'', array( dps_get_trash_status_id(), dps_get_spam_status_id() ) ) . "') AND post_type = '%s';", $forum_id, dps_get_topic_post_type() ) );

		// Update the count
		update_post_meta( $forum_id, '_dps_topic_count_hidden', (int) $topic_count );
	}

	return (int) apply_filters( 'dps_update_forum_topic_count_hidden', (int) $topic_count, $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_forum_query_subforum_ids() To get the subforum ids
 * @uses dps_update_forum_reply_count() To update the forum reply count
 * @uses dps_forum_query_topic_ids() To get the forum topic ids
 ...
 * @uses apply_filters() Calls 'dps_update_forum_topic_count' with the reply
 *                        count and forum id
 * @return int Forum reply count
 */
function dps_update_forum_reply_count( $forum_id = 0 ) {
	global $wpdb;
	...
	if ( !empty( $children ) ) {
		foreach ( (array) $children as $child ) {
			$children_reply_count += dps_update_forum_reply_count( $child );
		}
	}
	...
	$topic_ids = dps_forum_query_topic_ids( $forum_id );
	if ( !empty( $topic_ids ) ) {
		$reply_count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_parent IN ( " . join( ',', $topic_ids ) . " ) AND post_status = '%s' AND post_type = '%s';", dps_get_public_status_id(), dps_get_reply_post_type() ) );
	} else {
		$reply_count = 0;
	}
	...
	// Update the count
	update_post_meta( $forum_id, '_dps_reply_count',       (int) $reply_count   );
	update_post_meta( $forum_id, '_dps_total_reply_count', (int) $total_replies );

	return (int) apply_filters( 'dps_update_forum_reply_count', (int) $total_replies, $forum_id );
}
 ...
 * @uses dps_update_forum_last_topic_id() To update the forum last topic id
 * @uses dps_update_forum_last_reply_id() To update the forum last reply id
 * @uses dps_update_forum_last_active_id() To update the last active post id
 * @uses get_post_field() To get the post date of the last active id
 * @uses dps_update_forum_last_active_time()  To update the last active time
 * @uses dps_update_forum_subforum_count() To update the subforum count
 * @uses dps_update_forum_topic_count() To update the forum topic count
 * @uses dps_update_forum_reply_count() To update the forum reply count
 * @uses dps_update_forum_topic_count_hidden() To update the hidden topic count
 */
function dps_update_forum( $args = '' ) {
	...
	dps_update_forum_last_topic_id( $r['forum_id'], $r['last_topic_id'] );
	dps_update_forum_last_reply_id( $r['forum_id'], $r['last_reply_id'] );

	// Active dance
	$r['last_active_id'] = dps_update_forum_last_active_id( $r['forum_id'], $r['last_active_id'] );
	...
	if ( dps_get_public_status_id() == $r['last_active_status'] ) {
		dps_update_forum_last_active_time( $r['forum_id'], $r['last_active_time'] );
	}

	// Counts
	dps_update_forum_subforum_count    ( $r['forum_id'] );
	dps_update_forum_reply_count       ( $r['forum_id'] );
	dps_update_forum_topic_count       ( $r['forum_id'] );
	dps_update_forum_topic_count_hidden( $r['forum_id'] );

	// Update the parent forum if one was passed
	if ( !empty( $r['post_parent'] ) && is_numeric( $r['post_parent'] ) ) {
		dps_update_forum( array(
 ...
 * @uses apply_filters() Calls 'dps_forum_query_topic_ids' with the topic ids
 *                        and forum id
 */
function dps_get_hidden_forum_ids() {
   	$forum_ids = get_option( '_dps_hidden_forums', array() );

	return apply_filters( 'dps_get_hidden_forum_ids', (array) $forum_ids );
}
 ...
 *                        and forum id
 */
function dps_get_private_forum_ids() {
   	$forum_ids = get_option( '_dps_private_forums', array() );

	return apply_filters( 'dps_get_private_forum_ids', (array) $forum_ids );
}
 ...
 *
 * @uses dps_is_user_keymaster()
 * @uses dps_get_hidden_forum_ids()
 * @uses dps_get_private_forum_ids()
 * @uses apply_filters()
 */
function dps_exclude_forum_ids( $type = 'string' ) {
	...
	return apply_filters( 'dps_exclude_forum_ids', $retval, $forum_ids, $type );
}
 ...
 *
 * @uses apply_filters()
 * @uses dps_exclude_forum_ids()
 * @uses dps_get_topic_post_type()
 * @uses dps_get_reply_post_type()
 * @return WP_Query
 */
function dps_pre_get_posts_normalize_forum_visibility( $posts_query = null ) {

	// Bail if all forums are explicitly allowed
	if ( true === apply_filters( 'dps_include_all_forums', false, $posts_query ) ) {
		return;
	}
	...
	if ( in_array( dps_get_forum_post_type() , $post_types ) ) {
		...
		// Remove dps_get_private_status_id() if user is not capable
		if ( ! current_user_can( 'read_private_forums' ) ) {
			$key = array_search( dps_get_private_status_id(), $post_stati );
			if ( !empty( $key ) ) {
		...
		// Remove dps_get_hidden_status_id() if user is not capable
		if ( ! current_user_can( 'read_hidden_forums' ) ) {
			$key = array_search( dps_get_hidden_status_id(), $post_stati );
			if ( !empty( $key ) ) {
				unset( $post_stati[$key] );
			}
	...
	// Topics Or Replies
	if ( array_intersect( array( dps_get_topic_post_type(), dps_get_reply_post_type() ), $post_types ) ) {

		// Get forums to exclude
		$forum_ids = dps_exclude_forum_ids( 'meta_query' );
 ...
 * @uses dps_get_public_child_ids() To get the topic ids
 * @uses apply_filters() Calls 'dps_forum_query_topic_ids' with the topic ids
 *                        and forum id
 */
function dps_forum_query_topic_ids( $forum_id ) {
   	$topic_ids = dps_get_public_child_ids( $forum_id, dps_get_topic_post_type() );

	return apply_filters( 'dps_forum_query_topic_ids', $topic_ids, $forum_id );
}
 ...
 */
function _dps_forum_query_usort_subforum_ids( $a = 0, $b = 0 ) {
	$ta = get_post_meta( $a, '_dps_last_active_time', true );
	$tb = get_post_meta( $b, '_dps_last_active_time', true );
	return ( $ta < $tb ) ? -1 : 1;
}
 ...
 * @uses dps_forum_query_topic_ids() To get the forum's topic ids
 * @uses wpdb::prepare() To prepare the query
 * @uses wpdb::get_var() To execute the query and get the var back
 * @uses dps_get_reply_post_type() To get the reply post type
 * @uses wp_cache_set() To set the cache for future use
 * @uses apply_filters() Calls 'dps_forum_query_last_reply_id' with the reply id
 *                        and forum id
 */
function dps_forum_query_last_reply_id( $forum_id, $topic_ids = 0 ) {
	global $wpdb;
 ...
 * @uses dps_is_forum_hidden() To check if the forum is hidden or not
 * @uses dps_set_404() To set a 404 status
 */
function dps_forum_enforce_hidden() {

	// Bail if not viewing a single item or if user has caps
	if ( !is_singular() || dps_is_user_keymaster() || current_user_can( 'read_hidden_forums' ) )
		return;
	...
	switch ( $wp_query->get( 'post_type' ) ) {
		...
		case dps_get_topic_post_type() :
			$forum_id = dps_get_topic_forum_id( $wp_query->post->ID );
			break;

		// Reply
		case dps_get_reply_post_type() :
			$forum_id = dps_get_reply_forum_id( $wp_query->post->ID );
			break;

	}

	// If forum is explicitly hidden and user not capable, set 404
	if ( !empty( $forum_id ) && dps_is_forum_hidden( $forum_id ) && !current_user_can( 'read_hidden_forums' ) )
		dps_set_404();
}
...
function dps_forum_enforce_private() {
	...
	// Bail if not viewing a single item or if user has caps
	if ( !is_singular() || dps_is_user_keymaster() || current_user_can( 'read_private_forums' ) )
		return;
	...
	// If forum is explicitly hidden and user not capable, set 404
	if ( !empty( $forum_id ) && dps_is_forum_private( $forum_id ) && !current_user_can( 'read_private_forums' ) )
		dps_set_404();
}
...
function dps_check_forum_edit() {
	...
	if ( !current_user_can( 'edit_forum', dps_get_forum_id() ) ) {
		wp_safe_redirect( dps_get_forum_permalink() );
		exit();
	}
}
 ...
 * @uses dps_is_forum() To make sure it's a forum
 * @uses dps_get_topic_post_type() To get the topic post type
 * @uses dps_topics() To make sure there are topics to loop through
 ...
 */
function dps_delete_forum_topics( $forum_id = 0 ) {
 ...
 * @uses dps_get_forum_id() To validate the forum ID
 * @uses dps_is_forum() To make sure it's a forum
 * @uses dps_get_public_status_id() To return public post status
 * @uses dps_get_closed_status_id() To return closed post status
 * @uses dps_get_pending_status_id() To return pending post status
 * @uses dps_get_topic_post_type() To get the topic post type
 * @uses wp_trash_post() To trash the post
 * @uses update_post_meta() To update the forum meta of trashed topics
 * @return If forum is not valid
 */
function dps_trash_forum_topics( $forum_id = 0 ) {
	...
	$post_stati = join( ',', array(
		dps_get_public_status_id(),
		dps_get_closed_status_id(),
		dps_get_pending_status_id()
	) );
	...
	if ( !empty( $topics->posts ) ) {
		...
		update_post_meta( $forum_id, '_dps_pre_trashed_topics', $pre_trashed_topics );
 ...
 * @return If forum is not valid
 */
function dps_untrash_forum_topics( $forum_id = 0 ) {
	...
	$pre_trashed_topics = get_post_meta( $forum_id, '_dps_pre_trashed_topics', true );
 ...
 * @since Showcase (1.0)
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_delete_forum' with the forum id
 */
function dps_delete_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_delete_forum', $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_trash_forum' with the forum id
 */
function dps_trash_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_trash_forum', $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_untrash_forum' with the forum id
 */
function dps_untrash_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_untrash_forum', $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_deleted_forum' with the forum id
 */
function dps_deleted_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_deleted_forum', $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_trashed_forum' with the forum id
 */
function dps_trashed_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_trashed_forum', $forum_id );
}
 ...
 * @uses dps_get_forum_id() To get the forum id
 * @uses dps_is_forum() To check if the passed id is a forum
 * @uses do_action() Calls 'dps_untrashed_forum' with the forum id
 */
function dps_untrashed_forum( $forum_id = 0 ) {
	$forum_id = dps_get_forum_id( $forum_id );

	if ( empty( $forum_id ) || !dps_is_forum( $forum_id ) )
		return false;

	do_action( 'dps_untrashed_forum', $forum_id );
}
