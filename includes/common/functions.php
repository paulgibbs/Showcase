...
function dps_time_since( $older_date, $newer_date = false ) {
	...
	function dps_get_time_since( $older_date, $newer_date = false ) {
		...
		// Setup the strings
		$unknown_text   = apply_filters( 'dps_core_time_since_unknown_text',   __( 'sometime',  'barebones' ) );
		$right_now_text = apply_filters( 'dps_core_time_since_right_now_text', __( 'right now', 'barebones' ) );
		$ago_text       = apply_filters( 'dps_core_time_since_ago_text',       __( '%s ago',    'barebones' ) );
 ...
 */
function dps_get_redirect_to() {
	$retval = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : '';

	return apply_filters( 'dps_get_redirect_to', $retval );
}
 ...
 * @param bool $force Override dps_get_view_all() check
 * @uses current_user_can() To check if the current user can moderate
 * @uses add_query_arg() To add args to the url
 * @uses apply_filters() Calls 'dps_add_view_all' with the link and original link
 * @return string The link with 'view=all' appended if necessary
 */
function dps_add_view_all( $original_link = '', $force = false ) {

	// Are we appending the view=all vars?
	if ( dps_get_view_all() || !empty( $force ) ) {
		$link = add_query_arg( array( 'view' => 'all' ), $original_link );
	} else {
		$link = $original_link;
	}

	return apply_filters( 'dps_add_view_all', $link, $original_link );
}
 ...
 * @return string The link with 'view=all' appended if necessary
 */
function dps_remove_view_all( $original_link = '' ) {
	return apply_filters( 'dps_add_view_all', remove_query_arg( 'view', $original_link ), $original_link );
}
 ...
 *
 * @uses current_user_can() To check if the current user can moderate
 * @uses apply_filters() Calls 'dps_get_view_all' with the link and original link
 * @return bool Whether current user can and is viewing all
 */
function dps_get_view_all( $cap = 'moderate' ) {
	$retval = ( ( !empty( $_GET['view'] ) && ( 'all' == $_GET['view'] ) && current_user_can( $cap ) ) );
	return apply_filters( 'dps_get_view_all', (bool) $retval );
}
 ...
 * @uses dps_is_topic_anonymous() To check if the topic is by an anonymous user
 * @uses dps_is_reply_anonymous() To check if the reply is by an anonymous user
 * @return array Data
 */
function dps_fix_post_author( $data = array(), $postarr = array() ) {
	...
	if ( !in_array( $data['post_type'], array( dps_get_topic_post_type(), dps_get_reply_post_type() ) ) )
		return $data;

	// Is the post by an anonymous user?
	if ( ( dps_get_topic_post_type() == $data['post_type'] && !dps_is_topic_anonymous( $postarr['ID'] ) ) ||
	     ( dps_get_reply_post_type() == $data['post_type'] && !dps_is_reply_anonymous( $postarr['ID'] ) ) )
		return $data;
...
function dps_filter_anonymous_post_data( $args = '' ) {
	...
	if ( empty( $r['dps_anonymous_name'] ) )
		dps_add_error( 'dps_anonymous_name',  __( '<strong>ERROR</strong>: Invalid author name submitted!',   'barebones' ) );
	...
	if ( empty( $r['dps_anonymous_email'] ) )
		dps_add_error( 'dps_anonymous_email', __( '<strong>ERROR</strong>: Invalid email address submitted!', 'barebones' ) );
 ...
 * @uses dps_is_subscriptions_active() To check if the subscriptions are active
 * @uses dps_get_reply_id() To validate the reply ID
 * @uses dps_get_reply() To get the reply
 * @uses dps_get_reply_topic_id() To get the topic ID of the reply
 * @uses dps_is_reply_published() To make sure the reply is published
 * @uses dps_get_topic_id() To validate the topic ID
 * @uses dps_get_topic() To get the reply's topic
 * @uses dps_is_topic_published() To make sure the topic is published
 * @uses get_the_author_meta() To get the author's display name
 * @uses do_action() Calls 'dps_pre_notify_subscribers' with the reply id and
 *                    topic id
 * @uses dps_get_topic_subscribers() To get the topic subscribers
 * @uses apply_filters() Calls 'dps_subscription_mail_message' with the
 *                        message, reply id, topic id and user id
 * @uses get_userdata() To get the user data
 * @uses wp_mail() To send the mail
 * @uses do_action() Calls 'dps_post_notify_subscribers' with the reply id
 *                    and topic id
 * @return bool True on success, false on failure
 */
function dps_notify_subscribers( $reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = false, $reply_author = 0 ) {

	// Bail if subscriptions are turned off
	if ( !dps_is_subscriptions_active() )
		return false;

	/** Validation ************************************************************/

	$reply_id = dps_get_reply_id( $reply_id );
	$topic_id = dps_get_topic_id( $topic_id );
	...
	$user_ids = dps_get_topic_subscribers( $topic_id, true );
	if ( empty( $user_ids ) )
		return false;

	// Poster name
	$reply_author_name = dps_get_reply_author_display_name( $reply_id );

	/** Mail ******************************************************************/

	do_action( 'dps_pre_notify_subscribers', $reply_id, $topic_id, $user_ids );

	// Remove filters from reply content and topic title to prevent content
	// from being encoded with HTML entities, wrapped in paragraph tags, etc...
	remove_all_filters( 'dps_get_reply_content' );
	remove_all_filters( 'dps_get_topic_title'   );

	// Strip tags from text
	$topic_title   = strip_tags( dps_get_topic_title( $topic_id ) );
	$reply_content = strip_tags( dps_get_reply_content( $reply_id ) );
...
Login and visit the topic to unsubscribe from these emails.', 'barebones' ),
		...
		$message = apply_filters( 'dps_subscription_mail_message', $message, $reply_id, $topic_id, $user_id );
		if ( empty( $message ) )
			continue;

		// For plugins to filter titles per reply/topic/user
		$subject = apply_filters( 'dps_subscription_mail_title', '[' . $blog_name . '] ' . $topic_title, $reply_id, $topic_id, $user_id );
		if ( empty( $subject ) )
			continue;

		// Custom headers
		$headers = apply_filters( 'dps_subscription_mail_headers', array() );
	...
	do_action( 'dps_post_notify_subscribers', $reply_id, $topic_id, $user_ids );
 ...
 * @uses add_query_arg() To add args to the url
 * @uses apply_filters() Calls 'dps_logout_url' with the url and redirect to
 * @return string The url
 */
function dps_logout_url( $url = '', $redirect_to = '' ) {
	...
	// Filter and return
	return apply_filters( 'dps_logout_url', $url, $redirect_to );
}
 ...
 * @return string
 */
function dps_query_post_parent__in( $where, $object = '' ) {
	global $wpdb, $wp;
 ...
 * @return array
 */
function dps_request_feed_trap( $query_vars = array() ) {
	...
	if ( isset( $query_vars['feed'] ) ) {
		...
		if ( isset( $query_vars['post_type'] ) ) {
			...
			switch ( $query_vars['post_type'] ) {
				...
				case dps_get_forum_post_type() :
					...
					// Only forum replies
					if ( !empty( $_GET['type'] ) && ( dps_get_reply_post_type() == $_GET['type'] ) ) {
						...
						$the_query = array(
							...
							'posts_per_page' => dps_get_replies_per_rss_page(),
							'order'          => 'DESC',
							'meta_query'     => $meta_query
						);

						// Output the feed
						dps_display_replies_feed_rss2( $the_query );

					// Only forum topics
					} elseif ( !empty( $_GET['type'] ) && ( dps_get_topic_post_type() == $_GET['type'] ) ) {
						...
						$the_query = array(
							...
							'post_parent'    => dps_get_forum_id(),
							'post_status'    => array( dps_get_public_status_id(), dps_get_closed_status_id() ),
							'posts_per_page' => dps_get_topics_per_rss_page(),
							'order'          => 'DESC'
						);

						// Output the feed
						dps_display_topics_feed_rss2( $the_query );
					...
					// All forum topics and replies
						...
						if ( empty( $select_query_vars ) )
							$meta_query = array( dps_exclude_forum_ids( 'meta_query' ) );

						// The query
						$the_query = array(
							'author'         => 0,
							'feed'           => true,
							'post_type'      => array( dps_get_reply_post_type(), dps_get_topic_post_type() ),
							'post_parent'    => 'any',
							'post_status'    => array( dps_get_public_status_id(), dps_get_closed_status_id() ),
							'posts_per_page' => dps_get_replies_per_rss_page(),
							'order'          => 'DESC',
				...
				case dps_get_topic_post_type() :
					...
					if ( !empty( $select_query_vars ) ) {
						...
						// Output the feed
						dps_display_replies_feed_rss2( array( 'feed' => true ) );
		...
		// Single Topic Vview
		} elseif ( isset( $query_vars['dps_view'] ) ) {

			// Get the view
			$view = $query_vars['dps_view'];

			// We have a view to display a feed
			if ( !empty( $view ) ) {

				// Get the view query
				$the_query = dps_get_view_query_args( $view );
