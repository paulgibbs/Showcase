 ...
 */
function dps_code_trick( $content = '' ) {
	$content = str_replace( array( "\r\n", "\r" ), "\n", $content );
	$content = preg_replace_callback( "|(`)(.*?)`|",      'dps_encode_callback', $content );
	$content = preg_replace_callback( "!(^|\n)`(.*?)`!s", 'dps_encode_callback', $content );
 ...
 * @return string Partially encodedd content
 */
function dps_code_trick_reverse( $content = '' ) {

	// Setup variables
	$openers = array( '<p>', '<br />' );
	$content    = preg_replace_callback( "!(<pre><code>|<code>)(.*?)(</code></pre>|</code>)!s", 'dps_decode_callback', $content );
...
function dps_encode_bad( $content = '' ) {
	...
	foreach ( $allowed as $tag => $args ) {
		...
		if ( isset( $empty[$tag] ) ) {
			array_walk( $content, 'dps_encode_empty_callback',  $preg );
		} else {
			array_walk( $content, 'dps_encode_normal_callback', $preg );
		}
	}
 ...
 * @return string
 */
function dps_encode_callback( $matches = array() ) {
 ...
 * @return string
 */
function dps_decode_callback( $matches = array() ) {
 ...
 * @since Showcase (1.0)
 *
 * @internal Used by dps_encode_bad()
 * @param string $content
 * @param string $key Not used
 * @param string $preg
 */
function dps_encode_empty_callback( &$content = '', $key = '', $preg = '' ) {
	if ( strpos( $content, '`' ) !== 0 ) {
 ...
 * @param type $preg
 */
function dps_encode_normal_callback( &$content = '', $key = '', $preg = '') {
	if ( strpos( $content, '`' ) !== 0 ) {
