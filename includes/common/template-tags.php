 ...
 * @since Showcase (1.0)
 *
 * @uses dps_get_forums_url() To get the forums URL
 * @param string $path Additional path with leading slash
 */
function dps_forums_url( $path = '/' ) {
	echo dps_get_forums_url( $path );
}
	/**
	 ...
	 * @uses dps_get_root_slug() To get the forum root location
	 * @param string $path Additional path with leading slash
	 */
	function dps_get_forums_url( $path = '/' ) {
		return home_url( dps_get_root_slug() . $path );
	}
 ...
 * @since Showcase (1.0)
 *
 * @uses dps_get_topics_url() To get the topics URL
 * @param string $path Additional path with leading slash
 */
function dps_topics_url( $path = '/' ) {
	echo dps_get_topics_url( $path );
}
	/**
	 * Return the forum URL
	 *
	 ...
	 *
	 * @uses home_url() To get the home URL
	 * @uses dps_get_topic_archive_slug() To get the topics archive location
	 * @param string $path Additional path with leading slash
	 * @return The URL to the topics archive
	 */
	function dps_get_topics_url( $path = '/' ) {
		return home_url( dps_get_topic_archive_slug() . $path );
	}
 ...
 */
function dps_is_site_public( $site_id = 0 ) {
	...
	return (bool) apply_filters( 'dps_is_site_public', $public, $site_id );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_forum_edit is true
 * @return bool True if it's the forum edit page, false if not
 */
function dps_is_forum_edit() {
	...
	// Check query
	if ( !empty( $wp_query->dps_is_forum_edit ) && ( $wp_query->dps_is_forum_edit == true ) )
		$retval = true;
 ...
 */
function dps_is_topic( $post_id = 0 ) {

	// Assume false
	$retval = false;

	// Supplied ID is a topic
	if ( !empty( $post_id ) && ( dps_get_topic_post_type() == get_post_type( $post_id ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topic', $retval, $post_id );
}
 ...
 * @return bool
 */
function dps_is_single_topic() {

	// Assume false
	$retval = false;

	// Edit is not a single topic
	if ( dps_is_topic_edit() )
		return false;

	// Single and a match
	if ( is_singular( dps_get_topic_post_type() ) || dps_is_query_name( 'dps_single_topic' ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_topic', $retval );
}
 ...
 * @uses dps_get_topic_post_type() To get the topic post type ID
 *
 * @return bool
 */
function dps_is_topic_archive() {

	// Default to false
	$retval = false;

	// In topic archive
	if ( is_post_type_archive( dps_get_topic_post_type() ) || dps_is_query_name( 'dps_topic_archive' ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topic_archive', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_topic_edit is true
 * @return bool True if it's the topic edit page, false if not
 */
function dps_is_topic_edit() {
	global $wp_query, $pagenow;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_topic_edit ) && ( $wp_query->dps_is_topic_edit == true ) )
		$retval = true;

	// Editing in admin
	elseif ( is_admin() && ( 'post.php' == $pagenow ) && ( get_post_type() == dps_get_topic_post_type() ) && ( !empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topic_edit', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses dps_is_topic_edit() To check if it's a topic edit page
 * @return bool True if it's the topic merge page, false if not
 */
function dps_is_topic_merge() {

	// Assume false
	$retval = false;

	// Check topic edit and GET params
	if ( dps_is_topic_edit() && !empty( $_GET['action'] ) && ( 'merge' == $_GET['action'] ) )
		return true;

	return (bool) apply_filters( 'dps_is_topic_merge', $retval );
}
 ...
 */
function dps_is_topic_split() {

	// Assume false
	$retval = false;

	// Check topic edit and GET params
	if ( dps_is_topic_edit() && !empty( $_GET['action'] ) && ( 'split' == $_GET['action'] ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topic_split', $retval );
}
 ...
 */
function dps_is_topic_tag() {

	// Bail if topic-tags are off
	if ( ! dps_allow_topic_tags() )
		return false;

	// Return false if editing a topic tag
	if ( dps_is_topic_tag_edit() )
		return false;
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_topic_tag_edit is true
 * @return bool True if editing a topic tag, false if not
 */
function dps_is_topic_tag_edit() {
	...
	// Check query
	if ( !empty( $wp_query->dps_is_topic_tag_edit ) && ( true == $wp_query->dps_is_topic_tag_edit ) )
		$retval = true;

	// Editing in admin
	elseif ( is_admin() && ( 'edit-tags.php' == $pagenow ) && ( dps_get_topic_tag_tax_id() == $taxnow ) && ( !empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topic_tag_edit', $retval );
}
...
function dps_is_reply( $post_id = 0 ) {
	...
	if ( !empty( $post_id ) && ( dps_get_reply_post_type() == get_post_type( $post_id ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_reply', $retval, $post_id );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_reply_edit is true
 * @return bool True if it's the reply edit page, false if not
 */
function dps_is_reply_edit() {
	global $wp_query, $pagenow;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_reply_edit ) && ( true == $wp_query->dps_is_reply_edit ) )
		$retval = true;

	// Editing in admin
	elseif ( is_admin() && ( 'post.php' == $pagenow ) && ( get_post_type() == dps_get_reply_post_type() ) && ( !empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_reply_edit', $retval );
}

/**
 * Check if current page is a reply move page
 *
 * @uses dps_is_reply_move() To check if it's a reply move page
 * @return bool True if it's the reply move page, false if not
 */
function dps_is_reply_move() {

	// Assume false
	$retval = false;

	// Check reply edit and GET params
	if ( dps_is_reply_edit() && !empty( $_GET['action'] ) && ( 'move' == $_GET['action'] ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_reply_move', $retval );
}
 ...
 * @return bool
 */
function dps_is_single_reply() {

	// Assume false
	$retval = false;

	// Edit is not a single reply
	if ( dps_is_reply_edit() )
		return false;

	// Single and a match
	if ( is_singular( dps_get_reply_post_type() ) || ( dps_is_query_name( 'dps_single_reply' ) ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_reply', $retval );
}
...
function dps_is_favorites() {
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_favs ) && ( true == $wp_query->dps_is_single_user_favs ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_favorites', $retval );
}
...
function dps_is_subscriptions() {
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_subs ) && ( true == $wp_query->dps_is_single_user_subs ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_subscriptions', $retval );
}
 ...
 */
function dps_is_topics_created() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_topics ) && ( true == $wp_query->dps_is_single_user_topics ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_topics_created', $retval );
}
 ...
 * @return bool True if it's the topics created page, false if not
 */
function dps_is_replies_created() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_replies ) && ( true == $wp_query->dps_is_single_user_replies ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_replies_created', $retval );
}
...
function dps_is_user_home() {
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_home ) && ( true == $wp_query->dps_is_single_user_home ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_user_home', $retval );
}
 ...
 *
 * @since Showcase (1.0)
 * @uses dps_is_single_user_edit() Check query variable
 * @uses is_user_logged_in() Must be logged in to be home
 * @uses dps_get_displayed_user_id()
 * @uses dps_get_current_user_id()
 * @return bool True if it's the user's home, false if not
 */
function dps_is_user_home_edit() {

	// Assume false
	$retval = false;

	if ( dps_is_user_home() && dps_is_single_user_edit() )
		$retval = true;

	return (bool) apply_filters( 'dps_is_user_home_edit', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_single_user is set to true
 * @return bool True if it's a user's profile page, false if not
 */
function dps_is_single_user() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user ) && ( true == $wp_query->dps_is_single_user ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_user', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_single_user_edit is set to true
 * @return bool True if it's a user's profile edit page, false if not
 */
function dps_is_single_user_edit() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_edit ) && ( true == $wp_query->dps_is_single_user_edit ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_user_edit', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_single_user_profile is set to true
 * @return bool True if it's a user's profile page, false if not
 */
function dps_is_single_user_profile() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_single_user_profile ) && ( true == $wp_query->dps_is_single_user_profile ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_user_profile', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_single_user_topics is set to true
 * @return bool True if it's a user's topics page, false if not
 */
function dps_is_single_user_topics() {
	global $wp_query;
	...
	return (bool) apply_filters( 'dps_is_single_user_topics', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_single_user_replies is set to true
 * @return bool True if it's a user's replies page, false if not
 */
function dps_is_single_user_replies() {
	global $wp_query;
	...
	return (bool) apply_filters( 'dps_is_single_user_replies', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @global WP_Query $wp_query To check if WP_Query::dps_is_view is true
 * @uses dps_is_query_name() To get the query name
 * @return bool Is it a view page?
 */
function dps_is_single_view() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_view ) && ( true == $wp_query->dps_is_view ) )
		$retval = true;

	// Check query name
	if ( empty( $retval ) && dps_is_query_name( 'dps_single_view' ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_single_view', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @global WP_Query $wp_query To check if WP_Query::dps_is_search is true
 * @uses dps_is_query_name() To get the query name
 * @return bool Is it a search page?
 */
function dps_is_search() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_search ) && ( true == $wp_query->dps_is_search ) )
		$retval = true;

	// Check query name
	if ( empty( $retval ) && dps_is_query_name( 'dps_search' ) )
		$retval = true;

	// Check $_GET
	if ( empty( $retval ) && isset( $_GET[dps_get_search_rewrite_id()] ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_search', $retval );
}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Query Checks if WP_Query::dps_is_edit is true
 * @return bool True if it's the edit page, false if not
 */
function dps_is_edit() {
	global $wp_query;
	...
	// Check query
	if ( !empty( $wp_query->dps_is_edit ) && ( $wp_query->dps_is_edit == true ) )
		$retval = true;

	return (bool) apply_filters( 'dps_is_edit', $retval );
}
 ...
 * @uses apply_filters() Calls 'dps_wp_login_action' with the url and args
 */
function dps_wp_login_action( $args = '' ) {
	...
	echo apply_filters( 'dps_wp_login_action', $login_url, $r );
}
 ...
 * check the field manually, use dps_get_redirect_to().
 *
 * @since Showcase (1.0)
 *
 ...
 * @uses apply_filters() Calls 'dps_redirect_to_field', passes field and to
 */
function dps_redirect_to_field( $redirect_to = '' ) {
	...
	$redirect_field = '<input type="hidden" id="dps_redirect_to" name="redirect_to" value="' . $redirect_to . '" />';

	echo apply_filters( 'dps_redirect_to_field', $redirect_field, $redirect_to );
}
 ...
 * @uses dps_get_sanitize_val() To sanitize the value.
 */
function dps_sanitize_val( $request = '', $input_type = 'text' ) {
	echo dps_get_sanitize_val( $request, $input_type );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'dps_get_sanitize_val' with the sanitized
	 *                        value, request and input type
	 * @return string Sanitized value ready for screen display
	 */
	function dps_get_sanitize_val( $request = '', $input_type = 'text' ) {
		...
		return apply_filters( 'dps_get_sanitize_val', $retval, $request, $input_type );
	}
 ...
 * @since Showcase (1.0)
 *
 * @param mixed $args See {@link dps_get_dropdown()} for arguments
 */
function dps_dropdown( $args = '' ) {
	echo dps_get_dropdown( $args );
}
	/**
	...
	function dps_get_dropdown( $args = '' ) {
		...
		$r = dps_parse_args( $args, array(
			...
			// Output-related
			'select_id'          => 'dps_forum_id',
			'tab'                => dps_get_tab_index(),
			'options_only'       => false,
			'show_none'          => false,
			'none_found'         => false,
		...
		return apply_filters( 'dps_get_dropdown', $retval, $r );
	}
 ...
 * @uses dps_forum_id() To output the forum id
 * @uses dps_is_single_forum() To check if it's a forum page
 * @uses dps_forum_id() To output the forum id
 */
function dps_forum_form_fields() {

	if ( dps_is_forum_edit() ) : ?>

		<input type="hidden" name="action"       id="dps_post_action" value="bbp-edit-forum" />
		<input type="hidden" name="dps_forum_id" id="dps_forum_id"    value="<?php dps_forum_id(); ?>" />

		<?php

		if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-forum_' . dps_get_forum_id(), '_dps_unfiltered_html_forum', false );

		?>

		<?php wp_nonce_field( 'bbp-edit-forum_' . dps_get_forum_id() );

	else :

		if ( dps_is_single_forum() ) : ?>

			<input type="hidden" name="dps_forum_parent_id" id="dps_forum_parent_id" value="<?php dps_forum_parent_id(); ?>" />

		<?php endif; ?>

		<input type="hidden" name="action" id="dps_post_action" value="bbp-new-forum" />

		<?php

		if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-forum_new', '_dps_unfiltered_html_forum', false );
 ...
 * @uses wp_nonce_field() To generate hidden nonce fields
 * @uses dps_topic_id() To output the topic id
 * @uses dps_is_single_forum() To check if it's a forum page
 * @uses dps_forum_id() To output the forum id
 */
function dps_topic_form_fields() {

	if ( dps_is_topic_edit() ) : ?>

		<input type="hidden" name="action"       id="dps_post_action" value="bbp-edit-topic" />
		<input type="hidden" name="dps_topic_id" id="dps_topic_id"    value="<?php dps_topic_id(); ?>" />

		<?php

		if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-topic_' . dps_get_topic_id(), '_dps_unfiltered_html_topic', false );

		?>

		<?php wp_nonce_field( 'bbp-edit-topic_' . dps_get_topic_id() );
		...
		<input type="hidden" name="action" id="dps_post_action" value="bbp-new-topic" />

		<?php if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-topic_new', '_dps_unfiltered_html_topic', false ); ?>
 ...
 * @since Showcase (1.0)
 *
 * @uses dps_is_reply_edit() To check if it's the reply edit page
 * @uses wp_nonce_field() To generate hidden nonce fields
 * @uses dps_reply_id() To output the reply id
 * @uses dps_topic_id() To output the topic id
 * @uses dps_forum_id() To output the forum id
 */
function dps_reply_form_fields() {

	if ( dps_is_reply_edit() ) : ?>

		<input type="hidden" name="dps_reply_title" id="dps_reply_title" value="<?php dps_reply_title(); ?>" />
		<input type="hidden" name="dps_reply_id"    id="dps_reply_id"    value="<?php dps_reply_id(); ?>" />
		<input type="hidden" name="action"          id="dps_post_action" value="bbp-edit-reply" />

		<?php if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-reply_' . dps_get_reply_id(), '_dps_unfiltered_html_reply', false ); ?>

		<?php wp_nonce_field( 'bbp-edit-reply_' . dps_get_reply_id() );
		...
		<?php if ( current_user_can( 'unfiltered_html' ) )
			wp_nonce_field( 'bbp-unfiltered-html-reply_' . dps_get_topic_id(), '_dps_unfiltered_html_reply', false ); ?>

		<?php wp_nonce_field( 'bbp-new-reply' );

		// Show redirect field if not viewing a specific topic
		if ( dps_is_query_name( 'dps_single_topic' ) ) :
			dps_redirect_to_field( get_permalink() );
 ...
 * @since Showcase (1.0)
 *
 * @uses dps_displayed_user_id() To output the displayed user id
 * @uses wp_nonce_field() To generate a hidden nonce field
 * @uses wp_referer_field() To generate a hidden referer field
 */
function dps_edit_user_form_fields() {
?>

	<input type="hidden" name="action"  id="dps_post_action" value="bbp-update-user" />
	<input type="hidden" name="user_id" id="user_id"         value="<?php dps_displayed_user_id(); ?>" />

	<?php wp_nonce_field( 'update-user_' . dps_get_displayed_user_id() );
}
 ...
 * @uses dps_topic_id() To output the topic id
 */
function dps_merge_topic_form_fields() {
?>

	<input type="hidden" name="action"       id="dps_post_action" value="bbp-merge-topic" />
	<input type="hidden" name="dps_topic_id" id="dps_topic_id"    value="<?php dps_topic_id(); ?>" />

	<?php wp_nonce_field( 'bbp-merge-topic_' . dps_get_topic_id() );
}
 ...
 * @uses wp_nonce_field() To generate a hidden nonce field
 */
function dps_split_topic_form_fields() {
?>

	<input type="hidden" name="action"       id="dps_post_action" value="bbp-split-topic" />
	<input type="hidden" name="dps_reply_id" id="dps_reply_id"    value="<?php echo absint( $_GET['reply_id'] ); ?>" />

	<?php wp_nonce_field( 'bbp-split-topic_' . dps_get_topic_id() );
}
 ...
 * @uses wp_nonce_field() To generate a hidden nonce field
 */
function dps_move_reply_form_fields() {
?>

	<input type="hidden" name="action"       id="dps_post_action" value="bbp-move-reply" />
	<input type="hidden" name="dps_reply_id" id="dps_reply_id"    value="<?php echo absint( $_GET['reply_id'] ); ?>" />

	<?php wp_nonce_field( 'bbp-move-reply_' . dps_get_reply_id() );
}
 ...
 *
 * @param array $args
 * @uses dps_get_the_content() To return the content to output
 */
function dps_the_content( $args = array() ) {
	echo dps_get_the_content( $args );
}
	/**
	 ...
	 * @uses dps_use_wp_editor() To see if WP editor is in use
	 * @uses dps_is_edit() To see if we are editing something
	 * @uses wp_editor() To output the WordPress editor
	 *
	 * @return string HTML from output buffer
	 */
	function dps_get_the_content( $args = array() ) {
		...
		$r = dps_parse_args( $args, array(
			...
			'media_buttons'     => false,
			'textarea_rows'     => '12',
			'tabindex'          => dps_get_tab_index(),
			'tabfocus_elements' => 'dps_topic_title,dps_topic_tags',
		...
		$post_content = call_user_func( 'dps_get_form_' . $r['context'] . '_content' );
		...
		if ( dps_use_wp_editor() ) :

			// Enable additional TinyMCE plugins before outputting the editor
			add_filter( 'tiny_mce_plugins',   'dps_get_tiny_mce_plugins'   );
			add_filter( 'teeny_mce_plugins',  'dps_get_tiny_mce_plugins'   );
			add_filter( 'teeny_mce_buttons',  'dps_get_teeny_mce_buttons'  );
			add_filter( 'quicktags_settings', 'dps_get_quicktags_settings' );

			// Output the editor
			wp_editor( htmlspecialchars_decode( $post_content, ENT_QUOTES ), 'dps_' . $r['context'] . '_content', array(
				'wpautop'           => $r['wpautop'],
			...
			remove_filter( 'tiny_mce_plugins',   'dps_get_tiny_mce_plugins'   );
			remove_filter( 'teeny_mce_plugins',  'dps_get_tiny_mce_plugins'   );
			remove_filter( 'teeny_mce_buttons',  'dps_get_teeny_mce_buttons'  );
			remove_filter( 'quicktags_settings', 'dps_get_quicktags_settings' );
		 ...
		 */
			...
			<textarea id="dps_<?php echo esc_attr( $r['context'] ); ?>_content" class="<?php echo esc_attr( $r['editor_class'] ); ?>" name="dps_<?php echo esc_attr( $r['context'] ); ?>_content" cols="60" rows="<?php echo esc_attr( $r['textarea_rows'] ); ?>" tabindex="<?php echo esc_attr( $r['tabindex'] ); ?>"><?php echo $post_content; ?></textarea>
		...
		return apply_filters( 'dps_get_the_content', $output, $args, $post_content );
	}
 ...
 * @return array
 */
function dps_get_tiny_mce_plugins( $plugins = array() ) {
	...
	return apply_filters( 'dps_get_tiny_mce_plugins', $plugins );
}
 ...
 * @return array
 */
function dps_get_teeny_mce_buttons( $buttons = array() ) {
	...
	return apply_filters( 'dps_get_teeny_mce_buttons', $buttons );
}
 ...
 */
function dps_get_quicktags_settings( $settings = array() ) {
 ...
 * @uses dps_get_breadcrumb() To get the breadcrumb
 */
function dps_title_breadcrumb( $args = array() ) {
	echo dps_get_breadcrumb( $args );
}
 ...
 * @uses dps_get_breadcrumb() To get the breadcrumb
 */
function dps_breadcrumb( $args = array() ) {
	echo dps_get_breadcrumb( $args );
}
	/**
	...
	function dps_get_breadcrumb( $args = array() ) {
		...
		if ( apply_filters( 'dps_no_breadcrumb', is_front_page() ) )
			return;
		...
		if ( empty( $args['root_text'] ) ) {
			...
			$pre_root_text = dps_get_forum_archive_title();
		}
		...
		// Search page
		if ( dps_is_search() ) {
			$pre_current_text = dps_get_search_title();

		// Forum archive
		} elseif ( dps_is_forum_archive() ) {
			$pre_current_text = dps_get_forum_archive_title();

		// Topic archive
		} elseif ( dps_is_topic_archive() ) {
			$pre_current_text = dps_get_topic_archive_title();

		// View
		} elseif ( dps_is_single_view() ) {
			$pre_current_text = dps_get_view_title();

		// Single Forum
		} elseif ( dps_is_single_forum() ) {
			$pre_current_text = dps_get_forum_title();

		// Single Topic
		} elseif ( dps_is_single_topic() ) {
			$pre_current_text = dps_get_topic_title();

		// Single Topic
		} elseif ( dps_is_single_reply() ) {
			$pre_current_text = dps_get_reply_title();

		// Topic Tag (or theme compat topic tag)
		} elseif ( dps_is_topic_tag() || ( get_query_var( 'dps_topic_tag' ) && !dps_is_topic_tag_edit() ) ) {

			// Always include the tag name
			$tag_data[] = dps_get_topic_tag_name();
		...
		// Get post ancestors
		if ( is_singular() || dps_is_forum_edit() || dps_is_topic_edit() || dps_is_reply_edit() ) {
		...
		if ( !empty( $ancestors ) ) {
			...
			foreach( (array) $ancestors as $parent_id ) {
				...
				switch ( $parent->post_type ) {
					...
					// Topic
					case dps_get_topic_post_type() :
						$crumbs[] = '<a href="' . dps_get_topic_permalink( $parent->ID ) . '" class="bbp-breadcrumb-topic">' . dps_get_topic_title( $parent->ID ) . '</a>';
						break;

					// Reply (Note: not in most themes)
					case dps_get_reply_post_type() :
						$crumbs[] = '<a href="' . dps_get_reply_permalink( $parent->ID ) . '" class="bbp-breadcrumb-reply">' . dps_get_reply_title( $parent->ID ) . '</a>';
						break;
		...
		$sep    = apply_filters( 'dps_breadcrumb_separator', $sep    );
		$crumbs = apply_filters( 'dps_breadcrumbs',          $crumbs );

		// Build the trail
		$trail  = !empty( $crumbs ) ? ( $r['before'] . $r['crumb_before'] . implode( $sep . $r['crumb_after'] . $r['crumb_before'] , $crumbs ) . $r['crumb_after'] . $r['after'] ) : '';

		return apply_filters( 'dps_get_breadcrumb', $trail, $crumbs, $r );
	}
 ...
 * @since Showcase (1.0)
 *
 * @uses dps_get_allowed_tags()
 */
function dps_allowed_tags() {
	echo dps_get_allowed_tags();
}
	/**
	 ...
	 * @since Showcase (1.0)
	 *
	 * @uses dps_kses_allowed_tags() To get the allowed tags
	 * @uses apply_filters() Calls 'dps_allowed_tags' with the tags
	 * @return string HTML allowed tags entity encoded.
	 */
	function dps_get_allowed_tags() {

		$allowed = '';

		foreach ( (array) dps_kses_allowed_tags() as $tag => $attributes ) {
			$allowed .= '<' . $tag;
		...
		return apply_filters( 'dps_get_allowed_tags', htmlentities( $allowed ) );
	}
 ...
 * @since Showcase (1.0)
 *
 * @uses WP_Error Showcase::errors::get_error_codes() To get the error codes
 * @uses WP_Error Showcase::errors::get_error_data() To get the error data
 * @uses WP_Error Showcase::errors::get_error_messages() To get the error
 *                                                       messages
 ...
 * @uses dps_get_logout_link() To get the logout link
 */
function dps_logout_link( $redirect_to = '' ) {
	echo dps_get_logout_link( $redirect_to );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'dps_get_logout_link' with the logout link and
	 *                        redirect to url
