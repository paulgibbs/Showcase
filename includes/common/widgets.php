			...
			<form method="post" action="<?php dps_wp_login_action( array( 'context' => 'login_post' ) ); ?>" class="bbp-login-form">
				<fieldset>
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" name="user-submit" id="user-submit" tabindex="<?php dps_tab_index(); ?>" class="button submit user-submit"><?php _e( 'Log In', 'barebones' ); ?></button>
			...
			<?php while ( $widget_query->have_posts() ) : $widget_query->the_post(); ?>
				...
				<li>
					...
					if ( ( 'on' == $settings['show_date'] ) && !empty( $author_link ) ) :
						...
						printf( _x( '%1$s on %2$s %3$s', 'widgets', 'barebones' ), $author_link, $reply_link, '<div>' . dps_get_time_since( get_the_time( 'U' ) ) . '</div>' );

					// Reply link and timestamp
					elseif ( 'on' == $settings['show_date'] ) :

						// translators: 1: reply link, 2: reply timestamp
						printf( _x( '%1$s %2$s',         'widgets', 'barebones' ), $reply_link,  '<div>' . dps_get_time_since( get_the_time( 'U' ) ) . '</div>'              );
